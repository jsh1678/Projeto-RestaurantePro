...
class OrderController extends Controller
{
    ...
    public function updateStatus(Request $request, Order $order)
    {
        ...
        $request->validate(['status' => 'required|in:aberto,em_preparo,pronto,pronto_entrega,entregue,aguardando_pagamento,pago,cancelado']);

        $novoStatus = $request->status;

        // Quando o garçom marca como entregue, o pedido vai para aguardando_pagamento
        // A mesa permanece ocupada/bloqueada até o caixa confirmar o pagamento
        ...
        if ($novoStatus === 'aguardando_pagamento') {
            ...
            // Garante que a mesa fique como ocupada (aguardando pagamento)
            $order->table?->update(['status' => 'ocupada']);
        }

        $order->update($updates);

        return redirect()->route('dashboard')->with('success', 'Pedido marcado como entregue! Aguardando pagamento.');
    }
    ...
    public function cancelar(Order $order)
    {
        ...
        // Não pode cancelar se a conta já foi fechada (pronto_entrega ou aguardando_pagamento)
