...
class DashboardController extends Controller
{
    public function index(): View
    {
        ...
        return match($user->role) {
            ...
            'garcom' => view('dashboard.garcom', [
                'mesas' => Table::with(['orders' => fn($q) => $q->whereIn('status', ['aberto', 'em_preparo', 'pronto', 'pronto_entrega', 'aguardando_pagamento'])])->orderBy('numero')->get(),
                'pedidosGarcom' => Order::where('user_id', $user->id)->with('table', 'items')->orderByDesc('created_at')->take(30)->get(),
                'categorias' => Category::with('menuItems')->get(),
                'mesasOcupadas' => $mesasOcupadas,
                'totalMesas' => $totalMesas,
                'pedidosProntosPagamento' => Order::whereIn('status', ['pronto_entrega','aguardando_pagamento'])->with('table', 'items')->get(),
    ...
    public function vendas()
    {
        ...
        // Picos de venda: agrupa por dia e ordena pelos 3 maiores
        ...
        // Vendas agrupadas por dia para o gráfico de barras
