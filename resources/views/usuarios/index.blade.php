    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(320px,1fr)); gap:12px">
        ...
        <div style="background:var(--bg); border:1px solid var(--border); border-radius:12px; padding:16px; display:flex; align-items:center; gap:14px">
            ...
            <div style="display:flex; gap:6px; flex-shrink:0">
                ...
                @if($u->id !== Auth::id())
                {{-- [FIX #13] Modal em vez de confirm() nativo --}}
                <form method="POST" action="{{ route('usuarios.toggle', $u) }}" id="form-toggle-{{ $u->id }}">
                    @csrf @method('PATCH')
                    <button type="button"
                            class="btn btn-sm btn-icon {{ $u->ativo ? 'btn-warning' : 'btn-success' }}"
                            title="{{ $u->ativo ? 'Desativar' : 'Ativar' }} {{ $u->name }}"
                            aria-label="{{ $u->ativo ? 'Desativar' : 'Ativar' }} {{ $u->name }}"
                            onclick="confirmar(
                                '{{ $u->ativo ? 'Desativar' : 'Ativar' }} {{ addslashes($u->name) }}?',
                                function(){ document.getElementById('form-toggle-{{ $u->id }}').submit(); },
                                '{{ $u->ativo ? 'Desativar' : 'Ativar' }} Usuário'
                            )">
                        <i class="fa-solid {{ $u->ativo ? 'fa-ban' : 'fa-check' }}"></i>
                    </button>
                </form>

                <form method="POST" action="{{ route('usuarios.destroy', $u) }}" id="form-del-{{ $u->id }}">
                    @csrf @method('DELETE')
                    <button type="button"
                            ...
                            onclick="confirmar(
                                'Excluir {{ addslashes($u->name) }}? Esta ação não pode ser desfeita.',
                                function(){ document.getElementById('form-del-{{ $u->id }}').submit(); },
                                '⚠️ Excluir Usuário'
                            )">
